...
@Component({
  ...
  template: `
    <div class="space-y-1 relative" [class.z-50]="isOpen" [id]="dropdownId">
      ...
      <div *ngIf="isOpen" 
           class="absolute z-[9999] left-0 right-0 mt-1 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl shadow-2xl shadow-black/20 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200"
           [ngClass]="listClass">
        <div class="max-h-[200px] overflow-y-auto custom-scrollbar">
          <div *ngFor="let option of normalizedOptions" 
               (mousedown)="select($event, option.value)"
               (click)="select($event, option.value)"
               class="px-3 py-2 text-xs sm:text-sm cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-950/30 hover:text-blue-600 dark:hover:text-blue-400 transition-colors flex items-center justify-between group">
            <span class="font-medium" [class.capitalize]="autoCapitalize">{{ option.label }}</span>
            <i class="fa-solid fa-check text-[10px] opacity-0 group-hover:opacity-100 text-blue-600 dark:text-blue-400" 
