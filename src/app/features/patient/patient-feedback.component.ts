  ...
  template: `
    <app-patient-layout>
      <div class="min-h-screen bg-gray-50 dark:bg-[#0B1120] text-gray-900 dark:text-gray-100">
        <div class="max-w-7xl mx-auto p-4 sm:p-6 space-y-6 sm:space-y-10">
          ...
          <section>
            ...
            <!-- Empty State -->
            <div *ngIf="!loading && items.length === 0" class="text-center py-10 sm:py-16 bg-white dark:bg-gray-900/30 rounded-2xl border border-dashed border-gray-200 dark:border-gray-800">
              <div class="bg-gray-100 dark:bg-gray-800/50 w-12 h-12 sm:w-16 sm:h-16 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
                <i class="fa-solid fa-check text-xl sm:text-2xl text-green-500"></i>
              </div>
              <h3 class="text-base sm:text-lg font-medium text-gray-800 dark:text-gray-300">All caught up!</h3>
              <p class="text-xs sm:text-sm text-gray-600 dark:text-gray-500 mt-1 max-w-sm mx-auto">You have no pending feedback requests at the moment.</p>
            </div>
            ...
            <div *ngIf="!loading && items.length > 0" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3.5 sm:gap-6">
              ...
              >
                ...
                <!-- Appointment Info -->
                <div class="bg-gray-50 dark:bg-gray-950/50 rounded-lg p-2.5 sm:p-3 mb-3 sm:mb-6 border border-gray-200 dark:border-gray-800/50 flex items-center justify-between text-[11px] sm:text-xs text-gray-500 dark:text-gray-400">
                  <span class="flex items-center gap-1 sm:gap-1.5"><i class="fa-regular fa-calendar"></i> {{ a.appointmentDate }}</span>
                  <span class="flex items-center gap-1 sm:gap-1.5"><i class="fa-regular fa-clock"></i> {{ a.appointmentTime }}</span>
                  <span class="bg-gray-200 dark:bg-gray-800 px-1.5 sm:px-2 py-0.5 rounded text-[10px] text-gray-600 dark:text-gray-500">#{{ a.appointmentId }}</span>
                </div>
                ...
                <!-- Comment Input -->
                <div class="mb-3 sm:mb-4 relative group/input">
                  <textarea
                ...
                <!-- Error Message -->
                <div *ngIf="errors[a.appointmentId]" class="mb-3 p-2.5 bg-red-500/10 border border-red-500/20 rounded-lg flex items-center gap-2 text-xs text-red-400">
                  <i class="fa-solid fa-circle-exclamation"></i>
                  {{ errors[a.appointmentId] }}
                </div>

                <!-- Action Footer -->
                <div class="mt-auto pt-3 sm:pt-4 border-t border-gray-200 dark:border-gray-800 flex items-center justify-between">
                  ...
                  <div class="flex items-center gap-1.5 sm:gap-2">
                    <button 
                      (click)="skip(a)" 
                      class="px-2.5 sm:px-3 py-1 sm:py-1.5 text-xs font-medium text-gray-500 hover:text-gray-300 transition-colors uppercase tracking-wider"
                    >
                      Skip
                    </button>
                    <button 
          ...
          <!-- History Section -->
          <section class="mt-6 sm:mt-12 pt-4 sm:pt-8 border-t border-gray-200 dark:border-gray-800/50">
            <div class="flex items-center justify-between mb-4 sm:mb-8">
              <h2 class="text-base sm:text-xl font-semibold flex items-center gap-2 text-gray-800 dark:text-gray-300">
            ...
            <div *ngIf="givenVisible" class="animate-in fade-in slide-in-from-top-4 duration-300">
              ...
              <!-- Loading -->
              <div *ngIf="loadingGiven" class="flex flex-col items-center justify-center py-8 sm:py-12 text-gray-500">
                <i class="fa-solid fa-circle-notch fa-spin text-xl sm:text-2xl mb-2"></i>
                <span class="text-xs sm:text-sm">Retrieving history...</span>
              </div>

              <!-- Error -->
              <div *ngIf="!loadingGiven && givenError" class="p-3 sm:p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-xs sm:text-sm text-center">
                {{ givenError }}
              </div>

              <!-- Empty -->
              <div *ngIf="!loadingGiven && !givenError && givenFeedback.length === 0" class="text-center py-8 sm:py-12 text-xs sm:text-sm text-gray-500 bg-gray-100 dark:bg-gray-900/20 rounded-xl border border-dashed border-gray-200 dark:border-gray-800">
