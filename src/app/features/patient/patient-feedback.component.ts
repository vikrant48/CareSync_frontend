...
@Component({
  ...
  template: `
    <app-patient-layout>
      <div class="min-h-screen bg-gray-50 dark:bg-[#0B1120] text-gray-900 dark:text-gray-100">
        <div class="max-w-7xl mx-auto p-4 sm:p-6 space-y-10">
          ...
          <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-200 dark:border-gray-800 pb-6">
            ...
            <button 
              (click)="goBack()" 
              class="px-5 py-2.5 rounded-lg bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 transition-all duration-300 border border-gray-200 dark:border-gray-700 font-medium text-sm flex items-center justify-center gap-2 cursor-pointer w-full md:w-auto shadow-sm">
          ...
          <section>
            ...
            <!-- Loading State -->
            <div *ngIf="loading" class="flex flex-col items-center justify-center py-20 bg-gray-100 dark:bg-gray-900/50 rounded-2xl border border-gray-200 dark:border-gray-800/50">
              <i class="fa-solid fa-circle-notch fa-spin text-4xl text-blue-500 mb-4 h-10 w-10 flex items-center justify-center"></i>
              <p class="text-gray-500 dark:text-gray-400 animate-pulse">Loading appointments...</p>
            </div>

            <!-- Empty State -->
            <div *ngIf="!loading && items.length === 0" class="text-center py-16 bg-white dark:bg-gray-900/30 rounded-2xl border border-dashed border-gray-200 dark:border-gray-800">
              <div class="bg-gray-100 dark:bg-gray-800/50 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                <i class="fa-solid fa-check text-2xl text-green-500"></i>
              </div>
              <h3 class="text-lg font-medium text-gray-800 dark:text-gray-300">All caught up!</h3>
              <p class="text-gray-600 dark:text-gray-500 mt-1 max-w-sm mx-auto">You have no pending feedback requests at the moment.</p>
            </div>
            ...
            <div *ngIf="!loading && items.length > 0" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              <div 
                *ngFor="let a of items" 
                class="group bg-white dark:bg-gray-900/60 backdrop-blur-xl border border-gray-200 dark:border-gray-800 hover:border-blue-500/30 rounded-2xl p-6 transition-all duration-300 hover:shadow-xl hover:shadow-blue-900/10 flex flex-col relative overflow-hidden"
              >
                ...
                <div class="flex items-start justify-between mb-4 relative z-10">
                  <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-lg font-bold shadow-lg text-white">
                      {{ (a.doctorName[0] || 'D') | uppercase }}
                    </div>
                    <div>
                      <h3 class="font-bold text-lg text-gray-800 dark:text-gray-100 leading-tight">{{ a.doctorName }}</h3>
                      <p class="text-xs text-blue-500 dark:text-blue-400 font-medium uppercase tracking-wide mt-0.5">
                ...
                <!-- Appointment Info -->
                <div class="bg-gray-50 dark:bg-gray-950/50 rounded-lg p-3 mb-6 border border-gray-200 dark:border-gray-800/50 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <span class="bg-gray-200 dark:bg-gray-800 px-2 py-0.5 rounded text-[10px] text-gray-600 dark:text-gray-500">#{{ a.appointmentId }}</span>
                </div>
                ...
                <div class="mb-4 relative group/input">
                  <textarea
                    ...
                    rows="3"
                    class="w-full bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-3 text-sm text-gray-800 dark:text-gray-200 placeholder-gray-400 dark:placeholder-gray-600 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all resize-none outline-none"
                ...
                <!-- Action Footer -->
                <div class="mt-auto pt-4 border-t border-gray-200 dark:border-gray-800 flex items-center justify-between">
                  <label class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 cursor-pointer select-none">
                    <div class="relative flex items-center">
                      <input type="checkbox" [(ngModel)]="form[a.appointmentId].anonymous" class="peer h-4 w-4 opacity-0 absolute" />
                      <div class="w-4 h-4 bg-gray-200 dark:bg-gray-800 border border-gray-400 dark:border-gray-600 rounded peer-checked:bg-blue-500 peer-checked:border-blue-500 flex items-center justify-center transition-all">
          ...
          <!-- History Section -->
          <section class="mt-12 pt-8 border-t border-gray-200 dark:border-gray-800/50">
            <div class="flex items-center justify-between mb-8">
              <h2 class="text-xl font-semibold flex items-center gap-2 text-gray-800 dark:text-gray-300">
                <i class="fa-solid fa-clock-rotate-left"></i> History
              </h2>
              <button 
                (click)="toggleGiven()" 
                class="px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-800/50 hover:bg-gray-200 dark:hover:bg-gray-800 text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 border border-gray-200 dark:border-gray-700/50 hover:border-blue-500/30 transition-all text-sm font-medium"
              >
            ...
            <div *ngIf="givenVisible" class="animate-in fade-in slide-in-from-top-4 duration-300">
              ...
              <!-- Empty -->
              <div *ngIf="!loadingGiven && !givenError && givenFeedback.length === 0" class="text-center py-12 text-gray-500 bg-gray-100 dark:bg-gray-900/20 rounded-xl border border-dashed border-gray-200 dark:border-gray-800">
                No feedback history found.
              </div>

              <!-- Table -->
              <div *ngIf="!loadingGiven && givenFeedback.length > 0" class="bg-white dark:bg-gray-900/40 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden backdrop-blur-sm shadow-sm">
